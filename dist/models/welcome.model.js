...
let style = 'flex flex-col space-y-5 w-1/2 absolute top-1/2 left-1/2 -translate-1/2 text-center';
let btnStyle = 'cursor-pointer bg-purple-700 hover:bg-purple-800 px-5 py-2.5 mb-2';
let inputStyle = 'bg-transparent rounded p-2 placeholder-gray-400 outline-none text-center';
let value;
const welcomeComponent = createElement('div', { class: welcomeStyle });
const div = createElement('div', { class: style });
const input = createElement('input', { type: 'text', name: 'name', placeholder: "write your name...", class: inputStyle });
...
startGameBttn.addEventListener('click', () => {
    value = input.value;
    if (value.trim() === '')
        input.classList += ' border border-3 border-red-900';
    else {
