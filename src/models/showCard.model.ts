...
let previousEle: HTMLDivElement;
let isFliped: boolean = false;
let imgWon: number = 0;

const fail = new Audio("../assets/error.mp3");
const success = new Audio("../assets/good.wav");

export function showCard(currentElement: HTMLElement) {
  let card = currentElement.parentElement as HTMLDivElement;
...
export function resetCards() {
  ...
  for (let i = 0; i < cards.length; i++) cards[i]?.classList.remove("rotate-y-180");
}
